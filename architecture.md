...
**val_auc = 0.806 on ml-25m** | ~13M params | ~9.7 GB VRAM on NVIDIA L4
...
graph TD
    subgraph Inputs
        ...
        DENSE_RAW["Dense Features<br/>(timestamp, rating histograms,<br/>ug_dot, year, genre_count, movie_age)"]
        GENRE["Genre Multi-Hot"]
    end
    ...
    subgraph User-Side Sequential Modeling
        HIST_E --> CAUSAL["Causal Self-Attention<br/>(single head, Q/K/V linear)"]
        CAUSAL --> CONTEXTUAL["Contextual History<br/>(B, 100, 28)"]
        CONTEXTUAL --> DIN_CAT["Concat: contextual + ratings"]
        HIST_RAT --> DIN_CAT
        DIN_CAT --> DIN["DIN Attention<br/>(target-item-aware)<br/>Linear(336, 64) -> ReLU -> Linear(64, 1)"]
        ITEM_E -.->|target query| DIN
        DIN --> USER_HIST_E["user_hist_e<br/>dim=28"]
    end

    subgraph Item-Side DIN
        RATER_E --> IDIN_CAT["Concat: raters + ratings"]
        RATER_RAT --> IDIN_CAT
        IDIN_CAT --> IDIN["Item DIN Attention<br/>(target-user-aware)<br/>Linear(336, 64) -> ReLU -> Linear(64, 1)"]
        USER_E -.->|target query| IDIN
        IDIN --> ITEM_HIST_E["item_hist_e<br/>dim=28"]
    end

    subgraph Feature Processing
        DENSE_RAW --> BOTTOM_MLP["Bottom MLP<br/>17 -> 128 -> 28"]
    ...
    subgraph "GDCN Cross Network (4 layers)"
        ...
        GENRE_E --> CONCAT
        CONCAT --> X0["x0 (168)"]
        X0 --> GCL1["Gated Cross Layer 1<br/>cross = x0 * (W1 @ x0 + b1)<br/>gate = sigmoid(G1 @ x0)<br/>x1 = gate * cross + (1-gate) * x0"]
    ...
    subgraph "Top MLP -> Output"
        ...
        SIGMOID --> PRED["P(engage) in 0,1"]
    end

    subgraph "Training"
        LOGIT --> LOSS["BCEWithLogitsLoss<br/>label smoothing 0.1"]
        LOSS --> OPT["Adam, LR=1e-4<br/>weight_decay=1e-5<br/>AMP fp16, grad accum 8x"]
    end
