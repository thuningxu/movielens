...
_cache_path = Path(__file__).resolve().parent.parent / "data" / f"features_{_cache_key}.npz"
...
if _cache_path.exists():
    ...
    _genome_path = Path(__file__).resolve().parent.parent / "data" / DATASET / "genome-scores.csv"
    if _genome_path.exists():
        log.info("Loading tag genome scores...")
        if DATASET == "ml-25m":
            _raw = pd.read_csv(Path(__file__).resolve().parent.parent / "data" / DATASET / "ratings.csv")
        elif DATASET == "ml-10m":
            _raw = pd.read_csv(Path(__file__).resolve().parent.parent / "data" / "ml-10M100K" / "ratings.dat",
                               sep="::", engine="python",
                               names=["userId", "movieId", "rating", "timestamp"])
        elif DATASET == "ml-1m":
            _raw = pd.read_csv(Path(__file__).resolve().parent.parent / "data" / DATASET / "ratings.dat",
                               sep="::", engine="python",
                               names=["userId", "movieId", "rating", "timestamp"])
        else:
            _raw = None
    ...
    _tags_path = Path(__file__).resolve().parent.parent / "data" / DATASET / "tags.csv"
    if _tags_path.exists():
        ...
        _raw_ratings_path = Path(__file__).resolve().parent.parent / "data" / DATASET / "ratings.csv"
